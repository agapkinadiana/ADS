...
namespace FST
{
	bool step(FST& fst, short* &rstates)	//распознавание одного символа
	{
		bool rc = false;
		swap(rstates, fst.rstates);//меняем местами буферный и основной массив
		//цикл до кол сост
		for (short i = 0; i < fst.nstates; i++) {
			//если значение в массиве равно текущей позиции
			if (rstates[i] == fst.position)
			{
				//двигаемся по всем рёбрам нашей вершины
				for (short j = 0; j < fst.nodes[i].n_relation; j++) {
					//если символ перехода совпадаем с символом в строке
					if (fst.nodes[i].relations[j].symbol == fst.str[fst.position])
					{
						//записываем в буферный массив значение
	...
	bool execute(FST& fst)                          //распознавание цепочки
	{
		...
		memset(rstates, 0xff, sizeof(short)*fst.nstates);         //с помощье функц мемсет заполняем массив -1
		short lstring = fst.str.size();                           //определяем для нашей цепочки
		bool rc = true;
		for (short i = 0; i < lstring && rc; i++) {               //проходим по всем символам цепочки
			fst.position++;				                          //продвинули позицию
			rc = step(fst, rstates);	                          //функция step распознавание одного символа
		};
		delete[] rstates;
